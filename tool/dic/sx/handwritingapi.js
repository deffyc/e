...
QQShuru.Util.Event.getPoint = function(a) {
	if(QQShuru.Util.Browser.touchable){
		...
		lastY = lastY -top+document.body.scrollTop;
			//return [a.x, a.y]
			alert(Math.round(lastX)+","+ Math.round(lastY));
...
QQShuru.HWPanel = function(E, G) {
    ...
    var mousedown = function(W) {
        ...
        r++;
        if (o) {
            m(i, "losecapture", mouseup);
            i.setCapture()
        } else {
            m(window, "blur", mouseup)
        }
    };
    var mousemove = function(W) {
        if (!L) {
            return
        }
        var Y = B(W);
    ...
    var mouseup = function(W) {
        ...
        if (o) {
            j(i, "losecapture", mouseup);
            i.releaseCapture()
        } else {
            j(window, "blur", mouseup)
        }
        if (1 === u) {

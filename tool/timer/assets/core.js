...
function toggleLang() {
    ...
    if (lang) {
        if (lang.indexOf("zh") != -1) {
            $j.cookies.set("lang", "en", {
                expiresAt: d
            });
            selectLang("en");
        } else {
            $j.cookies.set("lang", "zh", {
                expiresAt: d
            });
            selectLang("zh", true);
        }
...
$j(document).ready(function() {
    ...
    if (lang) {
        ...
        var d = new Date();
        d.setTime(d.getTime() + (30 * 24 * 60 * 60 * 1000));
        $j.cookies.set("lang", window.navigator.userLanguage || window.navigator.language, {
            expiresAt: d
        });
...
var TimeUpSound;
soundManager.url = "/tool/timer/assets/ring/soundmanager2.swf";
...
soundManager.onready(function() {
    if (soundManager.supported()) {
        ...
        TimeUpSound = soundManager.createSound({
            id: 'aSound',
            url: '/tool/timer/assets/ring/ring.mp3',
            autoLoad: true,
            loops: 3
        });
    } else {
...
function toggleSwitchUntil() {
    if (isAfter) {
        isAfter = false;
        $j("#switchUntil").stop(true, true).animate({
            width: 94
        },
        300);
    } else {
...
function toggleSwitchNoLoop() {
    if (isLoop) {
        isLoop = false;
        $j("#switchNoLoop").stop(true, true).animate({
            width: 94
        },
        300);
    } else {
...
function refreshProgress() {
    ...
    $j("#progressBar").animate({
        width: (1 - (sec / totalSec)) * progressWidth
    },
    {
        queue: false,
        duration: 200

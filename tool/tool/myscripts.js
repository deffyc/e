...
var PlayTool = (function() {
    ...
    function loadSrc(audio, textPointer) {
        ...
        if (this.textPointer == -1) {
            ...
            ttsType=window.localStorage.getItem(TTS_STATIC)==null?"qita":window.localStorage.getItem(TTS_STATIC)
            audio.rel="noreferrer"
            audio.src = ttsMap.get(ttsType) + (ttsType=="qita"?encodeURI(text):encodeURI(text));
...
//https://blog.csdn.net/jack_rose_me/article/details/111628447
...
function setTTSType(){
    ...
    window.localStorage.setItem('ttsType',ttsType=="google"?"qita":"google");
